<script lang="ts">
  ...
  $effect(() => {
    ...
    const subscription = liveQuery(async () => {
      const dayLogs = await db.activity_logs.where('date').equals(dateStr).toArray();
  ...
  let activityStats = $derived.by(() => {
    return activities.map(activity => {
      const currentMinutes = logs.filter(l => l.activity_id === activity.id).reduce((sum, log) => sum + (log.duration_minutes || 0), 0);
      return {
        id: activity.id,
        name: activity.name,
    ...
    }).filter(stat => stat.targetMinutes > 0 || stat.currentMinutes > 0).sort((a, b) => b.currentMinutes - a.currentMinutes);
  });
...
<div class="px-4 py-6 pb-24 max-w-lg mx-auto">
  ...
  <div class="flex items-center justify-between mb-8">
    ...
    <a href="#/time/activities" class="text-sm font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 transition-colors">
      Manage Activities →
    </a>
  </div>
  <div class="flex items-center gap-2 bg-surface rounded-lg p-1 mb-8 justify-center">
    <button class="p-2 hover:bg-card rounded-md shadow-sm transition-all text-text-main" on:click={() => date = subDays(date, 1)}>←</button>
    <span class="font-medium px-2 min-w-[100px] text-center text-text-main">{format(date, 'MMM d')}</span>
    <button class="p-2 hover:bg-card rounded-md shadow-sm transition-all text-text-main" on:click={() => date = addDays(date, 1)}>→</button>
  </div>
  <!-- Timer Card -->
  ...
  <div class="space-y-4">
    ...
    {#each logs as log}
      <a href={`#/time/log/${log.id}`} class="bg-card p-4 rounded-2xl border border-border-subtle shadow-sm flex items-center justify-between hover:border-emerald-200 dark:hover:border-emerald-800 transition-colors block">
        <div>
    ...
    {/each}
    <a href={`#/time/add?date=${dateStr}`} class="block w-full py-4 rounded-2xl border-2 border-dashed border-zinc-200 text-zinc-400 font-medium text-center hover:border-zinc-300 hover:text-zinc-500 transition-all">
      + Log Activity
    </a>
  </div>
</div>
