<script lang="ts">
  ...
  let mealType = $page.url.searchParams.get('meal') || 'breakfast';
  let logId = $page.url.searchParams.get('log_id');
  ...
  onMount(async () => {
    if (logId) {
      ...
      if (log) {
        // Set meal type from log if distinct
        mealType = log.meal_type || mealType;
        
        const food = await db.foods.get(log.food_id);
        if (food) {
          selectedFood = food;
          // Determine if we should show gram/weight or serving
          // For now, let's default to 'serving' unless it was likely entered as weight?
          // We don't store "unit used" in log, only amount_consumed (servings).
          // But we can default to weight if it's a weight-based food, 
          // or just default to serving.
          
          const _isWeightBased = food.serving_unit && WEIGHT_BASED_REGEX.test(food.serving_unit);
          if (_isWeightBased) {
             // Let's default to weight unit for weight-based foods for better UX?
             // Or maybe stick to serving if that's what we have.
             // If I change to weight unit, I need to convert amount_consumed (servings) back to weight.
  ...
  // Calculated quantity (servings) for macros and saving
  ...
  function performSearch(query: string) {
    ...
    if (query.trim().length > 0) {
      ...
      // Show recent or popular foods? For now, empty or all
      subscription = liveQuery(async () => {
  ...
  function selectFood(food: Food) {
    ...
    const _isWeightBased = food.serving_unit && WEIGHT_BASED_REGEX.test(food.serving_unit);
    if (_isWeightBased) {
        selectedUnit = food.serving_unit!; // 'g', 'ml', or 'oz'
        inputValue = food.serving_size || 100; // Default to 1 serving's worth
    } else {
        selectedUnit = 'serving';
        inputValue = 1;
    }
  ...
  async function saveLog() {
    ...
    try {
      await db.logs.add({
        id: generateId(),
        user_id: 'local-user', // specific user id logic can be added later
        date: date,
        meal_type: mealType,
...
<div class="container mx-auto p-4 max-w-md pb-24">
  <div class="flex items-center mb-4">
    <a href="{base}/log?date={date}" class="mr-4 text-text-muted hover:text-text-main transition-colors">&larr; Back</a>
