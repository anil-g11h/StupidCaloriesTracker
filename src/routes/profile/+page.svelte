<script lang="ts">
	...
	// Active Goal: Get the most recent goal based on start_date
	const activeGoal = liveQueryStore(async () => {
		const goals = await db.goals.orderBy('start_date').reverse().toArray();
		return goals.length > 0 ? goals[0] : null;
	});

	// Reactively update form when activeData changes and we are not editing
	$effect(() => {
	...
	async function saveGoal() {
		...
		try {
			// Check if a goal already exists for today to update it, otherwise create new
	...
	async function addMetric() {
		try {
			const newMetric: BodyMetric = {
				...
				value: Number(metricForm.value), // Ensure number
			...
			// Reset value but keep date/type
			metricForm.value = 0;
		} catch (error) {
    ...
    // Update chart when weightHistory changes
	$effect(() => {
		if ($weightHistory && chartCanvas) {
			updateChart();
		}
	});

	onMount(() => {
        // Initial chart render if data is ready (might be handled by effect, but good to ensure)
		if ($weightHistory) {
            updateChart();
        }
	});
...
<div class="space-y-6 pb-20">
	...
	<div class="px-4 space-y-6 max-w-lg mx-auto">
		...
		<section class="bg-card p-5 rounded-xl shadow-sm border border-border-subtle">
			...
			{#if isEditingGoal}
				<form onsubmit={(e) => { e.preventDefault(); saveGoal(); }} class="space-y-4">
		...
		<section class="bg-card p-5 rounded-xl shadow-sm border border-border-subtle space-y-6">
			...
			<!-- Add Metric Form -->
			<form onsubmit={(e) => { e.preventDefault(); addMetric(); }} class="space-y-4 bg-surface p-4 rounded-xl border border-border-subtle">
				<div class="grid grid-cols-2 gap-4">
