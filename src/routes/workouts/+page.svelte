<script lang="ts">
  ...
  $effect(() => {
    const sub = liveQuery(() => db.workouts.orderBy('start_time').reverse().toArray()).subscribe(data => {
      workouts = data;
    });
...
<div class="pb-24 pt-4 px-4 max-w-md mx-auto">
  <header class="flex justify-between items-center mb-6">
    ...
    <a href="{base}/workouts/new" class="bg-brand text-white px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2 hover:bg-brand-dark transition-colors">
      <Plus size={18} />
      Start Workout
    </a>
  </header>
  ...
  {:else}
    <div class="space-y-3">
      {#each workouts as workout (workout.id)}
        <a href="{base}/workouts/{workout.id}" class="block bg-card rounded-xl p-4 shadow-sm border border-border-subtle hover:border-brand-light transition-colors">
            ...
            <!-- Summary of exercises could go here with a sub-query if we want to get fancy later -->
        </a>
      {/each}
    </div>
  {/if}
